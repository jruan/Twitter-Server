...
class Client(Iface):
  ...
  def testSubscribe(self, handle):
    ...
    Parameters:
     - handle
    """
    self.send_testSubscribe(handle)
    self.recv_testSubscribe()

  def send_testSubscribe(self, handle):
    self._oprot.writeMessageBegin('testSubscribe', TMessageType.CALL, self._seqid)
    args = testSubscribe_args()
    args.handle = handle
    args.write(self._oprot)
    self._oprot.writeMessageEnd()
    self._oprot.trans.flush()

  def recv_testSubscribe(self, ):
    (fname, mtype, rseqid) = self._iprot.readMessageBegin()
    if mtype == TMessageType.EXCEPTION:
      x = TApplicationException()
      x.read(self._iprot)
...
class Processor(Iface, TProcessor):
  def __init__(self, handler):
    ...
    self._processMap["testSubscribe"] = Processor.process_testSubscribe
  ...
  def process_testSubscribe(self, seqid, iprot, oprot):
    args = testSubscribe_args()
    args.read(iprot)
    iprot.readMessageEnd()
    result = testSubscribe_result()
    self._handler.testSubscribe(args.handle)
    oprot.writeMessageBegin("testSubscribe", TMessageType.REPLY, seqid)
    result.write(oprot)
    oprot.writeMessageEnd()
    oprot.trans.flush()
